...
const tags = ["Ludo", "Football", "Cricket", "Tea", "Brunch", "Shopping", "Instagram", "Study", "Music", "Photography",
    "Collecting", "Traveling", "Coffee", "Dancing", "Wine", "Manga", "Anime", "Memes", "Fashion", "Gym", "Drawing",
    "Boxing", "Walking", "Basketball", "Running", "Movies", "Web Series", "Cars", "Bike", "Maggi", "Sushi"]

const InterestSheet = ({ sheetRef, data, setData }) => {
    const { t } = useLanguage()
    const { colors } = useTheme()
    const Styles = styles(colors)
    const [search, setSearch] = useState('')
    const closeSheet = () => sheetRef.current.close()
    const notSelectedTags = tags?.filter(x => !data?.includes(x))
    ...
    // const resetSearch = () => {
    //     const notSelected = tags?.filter(x => !data?.includes(x))
    //     setSearchResult(notSelected)
    //     setSearch('')
    // }
...
export default memo(InterestSheet)
